...
def withvars(t, sigma):                                       # {{{1
  """Recursive substitute... replace replacements in replacements"""
  if isvar(t):
    if t.name not in sigma:
      return term(t)
    return withvars(term(sigma[t.name]), sigma)
  return t.copy(args = [ withvars(u, sigma) for u in t.args ])
